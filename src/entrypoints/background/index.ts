...
export default defineBackground({
    main: async () => {
        ...
        // Load storage and sync initial injections
        await storage.load();
        ...
        browser.storage.local.onChanged.addListener(
            useThrottleFn(async () => {
                ...
                for (const tab of tabs) {
                    ...
                    const matching = filterRulesByUrl(rules, tab.url).filter((r) => r.enabled);
        ...
        browser.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
            ...
            const matching = filterRulesByUrl(
                storage.rules as RuleT[],
                tab.url,
            ).filter((r) => r.enabled);
